...
async function requireAdmin(request: NextRequest) {
  ...
  if (!token) return { ok: false as const, status: 401, token: "" };

  const { anon, service } = getClients();
  const { data: userData } = await anon.auth.getUser(token);
  const requesterId = userData.user?.id ?? null;
  if (!requesterId) return { ok: false as const, status: 401, token: "" };
  ...
  if (!isAdmin) return { ok: false as const, status: 403, token: "" };
