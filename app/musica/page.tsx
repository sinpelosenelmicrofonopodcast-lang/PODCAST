...
export default async function MusicaPage() {
  ...
  let blogItems: BlogItem[] = [];
  {
    ...
    const needles = new Set(["musica", "music", "album", "cantante", "artista", "concierto"]);
    blogItems = normalizeCoverRows((data as BlogItem[]) ?? [])
      .filter((item) => {
        const cats = (item.categories ?? []).map(normalize);
        const tags = (item.tags ?? []).map(normalize);
        const bag = new Set([...cats, ...tags]);
        return Array.from(needles).some((needle) => bag.has(needle));
      })
      .slice(0, 12);
  }
        ...
        <div className="container" style={{ display: "grid", gap: 18 }}>
          <div className="card" style={{ display: "grid", gap: 10 }}>
            ...
            <p className="muted" style={{ margin: 0 }}>
              Noticias, análisis y contenido editorial del mundo musical.
            </p>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Link className="button secondary" href="/noticias?cat=M%C3%BAsica">
                Ver noticias de música
              </Link>
              <Link className="button secondary" href="/blog?cat=M%C3%BAsica">
                Ver blogs de música
              </Link>
            </div>
          </div>

          <MidContentAdSlot placement="section_header" section="musica" className="section-ad-slot" compact />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(320px, 100%), 1fr))", gap: 14 }}>
            <div className="card">
              ...
              {newsItems.length === 0 ? <p className="muted">No hay noticias de música todavía.</p> : null}
              <div className="list">
                {newsItems.map((item) => (
            ...
            <div className="card">
              ...
              {blogItems.length === 0 ? <p className="muted">No hay blogs de música todavía.</p> : null}
              <div className="list">
                {blogItems.map((item) => (
                  <article key={item.id} className="card">
                    {item.cover_url ? (
